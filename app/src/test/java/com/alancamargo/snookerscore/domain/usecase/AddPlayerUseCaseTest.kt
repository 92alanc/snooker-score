...
class AddPlayerUseCaseTest {
    ...
    private val useCase = AddPlayerUseCase(mockRepository)

    @Test
    fun `invoke should add new player`() = runBlocking {
        val player = Player(id = "1", name = "Mark Selby")
        every { mockRepository.addPlayer(player) } returns flow { emit(Unit) }
    ...
    fun `when repository throws exception invoke should return error`() = runBlocking {
        ...
        every { mockRepository.addPlayer(player) } returns flow { throw Throwable(errorMessage) }
